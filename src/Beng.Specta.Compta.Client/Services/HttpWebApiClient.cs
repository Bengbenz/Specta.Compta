...
public class HttpWebApiClient
{
    ...
    private void RedirectToLocal(string? navigateTo) => _navigationManager.NavigateTo(!string.IsNullOrWhiteSpace(navigateTo) ? navigateTo : "/");
    ...
    public HttpWebApiClient OnBadRequest<T>(Action<T?> action)
    {
        _onBadRequest = async (r) =>
        {
            var response = await JsonSerializer.DeserializeAsync<T>(await r.Content.ReadAsStreamAsync());
            action(response);
        };
        return this;
    }
